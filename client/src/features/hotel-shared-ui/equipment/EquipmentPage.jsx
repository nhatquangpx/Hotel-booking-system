...
/**
 * Trang thiết bị dùng chung owner / staff.
 *
 ...
 * @param {object} equipmentApi — getByHotel, post/patch/deleteEquipment, postRepairRequest
 * @param {object} maintenanceApi — getMaintenanceContact; updateMaintenanceContact nếu canEdit
 * @param {boolean} [canEditMaintenanceEmail=false]
...
export default function EquipmentPage({
  ...
  const [maintenanceEmailDraft, setMaintenanceEmailDraft] = useState('');
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [data, maint] = await Promise.all([
        equipmentApi.getByHotel(hotelId),
        maintenanceApi.getMaintenanceContact(hotelId),
      ]);
      if (seq !== equipmentFetchSeq.current) {
        return;
      }
      setRooms(Array.isArray(data.rooms) ? data.rooms : []);
      setMaintenanceEmailDraft(
  ...
  }, [hotelId, hotelLoading, equipmentApi, maintenanceApi]);
  ...
  const handleAddEquipment = async (roomId) => {
    ...
    try {
      const data = await equipmentApi.postEquipment(roomId, {
        name,
  ...
  const handleStatusChange = async (roomId, equipmentId, status) => {
    ...
    setUpdating(key, true);
    try {
      const data = await equipmentApi.patchEquipment(roomId, equipmentId, { status });
      setRooms((prev) => mergeRoomIntoList(prev, roomId, data));
    } catch (err) {
      toast.error(apiErrorMessage(err));
    } finally {
  ...
  const handleNameBlur = async (roomId, equipmentId, currentName, newValue) => {
    ...
    setUpdating(key, true);
    try {
      const data = await equipmentApi.patchEquipment(roomId, equipmentId, { name: trimmed });
      setRooms((prev) => mergeRoomIntoList(prev, roomId, data));
      setNameEditKey(null);
  ...
  const handleSaveMaintenanceEmail = async () => {
    if (!hotelId || !canEditMaintenanceEmail || !maintenanceApi.updateMaintenanceContact) return;
    const email = maintenanceEmailDraft.trim();
    const emailOk = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email);
    if (email && !emailOk) {
      toast.warn('Email bên sửa chữa không hợp lệ.');
      return;
    }
    setMaintenanceSaving(true);
    try {
      const data = await maintenanceApi.updateMaintenanceContact(hotelId, {
        maintenanceContactEmail: email,
      });
      setMaintenanceEmailDraft(
        typeof data?.maintenanceContactEmail === 'string' ? data.maintenanceContactEmail : ''
      );
      toast.success('Đã lưu email bên sửa chữa.');
    } catch (err) {
      toast.error(apiErrorMessage(err));
    } finally {
  ...
  const closeRepairDialog = () => {
    if (repairSending || (canEditMaintenanceEmail && maintenanceSaving)) return;
    setRepairDialogOpen(false);
    setRepairSendError(null);
  };
  ...
  const sendRepairRequest = async () => {
    ...
    setRepairSendError(null);
    try {
      await equipmentApi.postRepairRequest(hotelId, { items });
  ...
  const confirmDeleteEquipment = async () => {
    ...
    setDeleteError(null);
    try {
      const data = await equipmentApi.deleteEquipment(roomId, equipmentId);
      setRooms((prev) => mergeRoomIntoList(prev, roomId, data));
  ...
  const showEmptyHotel = !hotelLoading && !hotelId;
  ...
  return (
    <Layout>
      <div className="owner-equipment-page">
        ...
        {showEmptyHotel && (
          <div className="owner-equipment-page__hint">{emptyHotelMessage}</div>
        )}

        {(error || (hotelId && hotelError)) && (
          <div className="owner-equipment-page__error">{error || hotelError}</div>
        )}
        ...
        {!loading && !hotelLoading && hotelId && rooms.length === 0 && !error && !hotelError && (
      ...
      <EquipmentRepairRequestDialog
        ...
        maintenanceEmailDraft={maintenanceEmailDraft}
        onMaintenanceEmailChange={
          canEditMaintenanceEmail
            ? (e) => setMaintenanceEmailDraft(e.target.value)
            : undefined
        }
        onSaveMaintenanceEmail={canEditMaintenanceEmail ? handleSaveMaintenanceEmail : undefined}
        maintenanceSaving={maintenanceSaving}
        canEditMaintenanceEmail={canEditMaintenanceEmail}
        pageLoading={loading}
