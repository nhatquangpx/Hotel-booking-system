...
/**
 * Owner — quản lý trang thiết bị theo phòng (thêm thủ công, không gộp từ tiện ích/dịch vụ).
 */
const OwnerEquipmentPage = () => {
  ...
  /** { roomId, equipmentId, equipmentName, roomNumber } | null */
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [data, maint] = await Promise.all([
        api.ownerEquipment.getByHotel(selectedHotelId),
        api.ownerHotel.getMaintenanceContact(selectedHotelId),
      ]);
  ...
  const handleAddEquipment = async (roomId) => {
    ...
    try {
      const data = await api.ownerEquipment.postEquipment(roomId, {
        name,
  ...
  const handleStatusChange = async (roomId, equipmentId, status) => {
    ...
    setUpdating(key, true);
    try {
      const data = await api.ownerEquipment.patchEquipment(roomId, equipmentId, { status });
      setRooms((prev) => mergeRoomIntoList(prev, roomId, data));
    } catch (err) {
      toast.error(apiErrorMessage(err));
    } finally {
  ...
  const handleNameBlur = async (roomId, equipmentId, currentName, newValue) => {
    ...
    setUpdating(key, true);
    try {
      const data = await api.ownerEquipment.patchEquipment(roomId, equipmentId, { name: trimmed });
      setRooms((prev) => mergeRoomIntoList(prev, roomId, data));
      setNameEditKey(null);
  ...
  const handleSaveMaintenanceEmail = async () => {
    ...
    try {
      const data = await api.ownerHotel.updateMaintenanceContact(selectedHotelId, {
  ...
  const closeRepairDialog = () => {
    if (repairSending || maintenanceSaving) return;
    setRepairDialogOpen(false);
    setRepairSendError(null);
  };
  ...
  const sendRepairRequest = async () => {
    ...
    setRepairSendError(null);
    try {
      await api.ownerEquipment.postRepairRequest(selectedHotelId, { items });
  ...
  const confirmDeleteEquipment = async () => {
    ...
    setDeleteError(null);
    try {
      const data = await api.ownerEquipment.deleteEquipment(roomId, equipmentId);
      setRooms((prev) => mergeRoomIntoList(prev, roomId, data));
  ...
  const showEmptyHotel = !hotelsLoading && !selectedHotelId;
  ...
  return (
    <OwnerLayout>
      <div className="owner-equipment-page">
        ...
        {showEmptyHotel && (
          <div className="owner-equipment-page__hint">Vui lòng chọn khách sạn ở thanh trên để xem danh sách phòng.</div>
        )}

        {error && <div className="owner-equipment-page__error">{error}</div>}
      ...
      <EquipmentRepairRequestDialog
        ...
        onMaintenanceEmailChange={(e) => setMaintenanceEmailDraft(e.target.value)}
        onSaveMaintenanceEmail={handleSaveMaintenanceEmail}
