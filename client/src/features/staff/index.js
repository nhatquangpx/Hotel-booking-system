/**
 * Staff Features — nhân viên khách sạn
 */
export { default as StaffDashboardPage } from './dashboard';
export { default as StaffRoomMapPage } from './rooms';
export { default as StaffBookingsPage } from './bookings';
export { default as StaffAddonServicesPage } from './addon';
export { default as StaffEquipmentPage } from './equipment';
export { default as StaffReviewsPage } from './reviews';
export { default as StaffNotificationsPage } from './notifications';
export {
  StaffProfileAccountPage,
  StaffProfileEditPage,
  StaffProfileChangePasswordPage,
} from './profile';
